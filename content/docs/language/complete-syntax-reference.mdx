...
```txt
tooltip "More information" trigger="Info"
```
...
### 4. Do not place `field` directly inside `dialog`
...
Wrong because `field` only collects data as part of a `form`.
...
### 5. Do not use unquoted field default values
...
Wrong because the parser reads field defaults from quoted `value="..."`, and React uses those parsed values as initial input values.

### 6. Do not emit incomplete strict nodes
